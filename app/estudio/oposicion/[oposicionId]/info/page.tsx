...
import { getOposicion } from "@/lib/temario";
import {
  getFaqEspecificas,
  getPerfilOposicion,
...
/**
 * Requisitos/funciones/retribución/participación/FAQs de una oposición —
 * extraído de app/estudio/oposicion/[oposicionId]/page.tsx a su propia
 * ruta (petición de Diego: esta información no debe ir por delante del
 * temario, que es lo primero que se quiere ver al entrar en una
 * oposición). Esa página enlaza aquí en vez de mostrarlo inline.
 */
export default async function InfoOposicionPage({
  params,
}: {
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-10">
      <p className="text-sm text-texto-secundario">
        <Link
          href={`/estudio/oposicion/${oposicionId}`}
          className="underline underline-offset-2 hover:text-texto-primario"
        >
          ← {oposicion.nombre}
        </Link>
      </p>
      <h1 className="mt-1 text-2xl font-semibold text-texto-primario">Información de la oposición</h1>
      <p className="mt-1 text-texto-secundario">
        {oposicion.organismo} · {oposicion.subgrupo}
      </p>
      ...
      {perfil ? (
        ...
        <p className="mt-8 text-texto-secundario">
          Todavía no hay información detallada de esta oposición en la app.
        </p>
      )}
    </main>
  );
