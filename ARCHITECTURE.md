...
- **Las anotaciones personales de lectura (`specs/020-anotaciones-personales-lectura`) siguen el mismo criterio y viven en `localStorage`** (`lib/anotaciones-lectura.ts`, clave `gsw_anotaciones:<conceptoId>:<seccionId>`, una entrada por cada una de las 4 secciones anotables de un concepto) — mismas dos razones que el progreso de lectura (crece con el temario, no tiene coste de parpadeo), más una tercera: es contenido HTML potencialmente grande (una sección anotada completa), no un único número.
...
### Extensión (2026-09-15): anotaciones personales de lectura (`specs/020-anotaciones-personales-lectura`)

Capa de anotación cliente-only sobre el contenido ya renderizado por `SeccionesConcepto.tsx` (negrita/subrayado/resaltado aplicados con `contentEditable` + Range API manual, sin librería de editor ni `document.execCommand`) — no toca `content/estudio/*.md` ni el pipeline `Markdown.tsx`/`rehype-sanitize` de texto enriquecido de autoría, son dos sistemas deliberadamente separados. Detalle completo de las decisiones en `specs/020-anotaciones-personales-lectura/design.md`.

- **Nuevo par de tokens `--anotacion-fondo`/`--anotacion-texto`** (`app/globals.css`), color propio (teal) distinto de los 6 pares `ink-*` de autoría — evita que una marca personal de Diego se confunda visualmente con un resaltado que puso el redactor del contenido. Contraste verificado ≥6.8:1 en los tres temas (claro/oscuro/papel).
- **Detección de contenido desactualizado vía checksum de una pasada** del `textContent` de la sección (`calcularHash`, `lib/anotaciones-lectura.ts`), no una librería de hashing — solo necesita detectar "esto cambió", no resistir colisiones intencionadas.
- **Dos bugs de Range API encontrados y corregidos durante la verificación en navegador** (no evidentes leyendo el código, solo probando selección real): `extractContents()` no clona el envoltorio de formato en el fragmento extraído cuando la selección cae dentro de un único nodo de texto (lo deja vacío en el DOM en su lugar) — `BarraFormato.tsx` (`purgarEnvoltoriosVacios`) limpia ese envoltorio vacío tanto si queda como ancestro como si queda como hermano del punto de inserción, según por dónde cayera el límite de la selección. Y la restauración de anotaciones al montar (`SeccionesConcepto.tsx`) protege la captura del HTML original frente a la doble invocación de efectos de StrictMode en desarrollo. Quedan como referencia porque el mismo patrón (`extractContents`/`insertNode` manual) se reutilizaría en cualquier ampliación futura de `BarraFormato`.
